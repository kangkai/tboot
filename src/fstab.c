...
void load_volume_table()
{
	...
	FILE *fstab = fopen(PREOS_FSTAB_LOCATION, "r");
	if (fstab == NULL) {
		pr_error("failed to open %s (%s)\n",
		     PREOS_FSTAB_LOCATION, strerror(errno));
		return;
	}
	...
	while (fgets(buffer, sizeof(buffer) - 1, fstab)) {
		...
		if (mount_point && fs_type && device) {
			...
			if (parse_options(options, device_volumes + num_volumes)
			    != 0) {
				pr_error("skipping malformed preos.fstab line: %s\n", buffer);
			} else {
				++num_volumes;
			}
		} else {
...
/*
 * basic check, if all device nodes exist, return 0
 */
static int devnode_rw(const char *fn)
{
	struct stat st;
...
int check_devnodes(void)
{
	...
	for (i = 0; i < num_volumes; i++) {
		v = &device_volumes[i];
		if (v->device && strlen(v->device) > 0 &&
				devnode_rw(v->device))
			return -1;
		if (v->device2 && strlen(v->device2) > 0 &&
				devnode_rw(v->device2))
			return -1;
	}
	return 0;
}
