...
find_mbr_part(struct disk_info *dinfo, const char *name)
{
    ...
    /* If the device name ends with a digit we need to insert a 'p'
     * when forming the node name of the specific partition,
     * such as with mmcblk devices */
    if (isdigit(dinfo->device[strlen(dinfo->device) - 1])) {
        device_pat = "%sp%d";
    } else {
        device_pat = "%s%d";
    }
    num = snprintf(dev_name, MAX_NAME_LEN, device_pat, dinfo->device, num);
    if (num >= MAX_NAME_LEN) {
